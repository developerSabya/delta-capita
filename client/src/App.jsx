...
export default function App() {
  ...
  return (
    <div className="app">
      <header className="header">PDF Signer (Mobile-friendly)</header>
      <main className="card">
        <label className="file-label">
          <input ref={inputRef} type="file" accept="application/pdf" onChange={onFileChange} />
          <div className="file-control">
            <div className="file-name">{file ? file.name : 'Choose a PDF'}</div>
            <button className="choose">Browse</button>
          </div>
        </label>

        <div style={{display:'flex',gap:8,marginTop:12}}>
          <button className="primary" onClick={uploadAndSign} disabled={!file || loading}>
      ...
      <footer className="footer">Mock signing server at {serverBase}</footer>
    </div>
  )
}
